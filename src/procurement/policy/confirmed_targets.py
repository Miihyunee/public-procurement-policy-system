...
⛔ 저장하는 것과 **달성률을 낼 수 있는 것**은 다릅니다. 경계는
:data:`~procurement.core.target_scope.CALCULABLE_SCOPES` 입니다.

* 총 구매금액 기준 6종 — 계산됩니다.
* 여성기업(구매유형별) — **STEP 103 으로 계산됩니다.** 담당자가 확정한 유형만
  세며, 확정이 없는 유형은 분모가 0 이라 «계산 보류» 로 남습니다.
* 자활용사촌(생산가능품목) — 거래별 품목 정보가 없어 여전히 «계산 보류» 입니다.

⛔ 없는 분모를 전체 구매금액으로 바꿔치기하지 않습니다.
...
#: 구매유형(``purchase_review.final_purchase_type``)을 분모·분자에 함께 적용하는
#: 경로가 생겨 유형별 달성률을 낼 수 있게 되었습니다. ⛔ 유형을 자동 판정하게 된
#: 것이 아니라 **확정된 행만** 셉니다 — 확정이 없으면 그 유형의 분모가 0 이 되고,
#: 그때는 0% 가 아니라 «계산 보류» 로 둡니다.
