...
class ReviewQuery:
    ...
    Attributes:
        search: **적요 · 거래처명 · 사업자등록번호 · 금액**. 빈 값이면 전체.
            띄어쓰기 차이는 무시합니다.

            결의번호가 없어 담당자가 이 값들로 지출결의서를 찾는다고 고객이
            답했습니다(2026-08-31 · Q5-3). 한 칸에서 함께 찾습니다.

            🟢 **금액**은 2026-08-31 고객 최종 회신(``DECISIONS.md`` §0.12.5 ·
            Q71-C)으로 더해졌습니다 — *"검토화면에서 금액, 사업자등록번호,
            적요 정도는 검색기능이 있으면 좋겠어."*

            ⛔ 금액은 **정확히 같은 값**만 찾습니다(범위·근사 없음). 금액으로
            읽히지 않는 글자는 금액 조건이 되지 않습니다 —
            :func:`~procurement.core.amount_search.amount_search_key`.

            ⛔ **고객이 말한 것은 이 셋뿐입니다.** 다른 검색 조건을 함께
            만들지 않았습니다.
