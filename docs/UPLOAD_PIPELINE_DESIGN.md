...
# 4.7 🔴 재업로드 교체 — 기존 구조 분석 (2026-08-18, 구현 전)
...
## 4.7.2 🔴 지금 없는 것 — 사용자 확인 팝업 (PM-005 미충족)

**현재 `POST /uploads/purchases` 는 묻지 않고 교체한다.** 실기동에서 확인했다.

```text
같은 연도로 두 번 업로드 →
  "배치 #2 로 2건을 저장했습니다. 같은 기간의 이전 배치 #1 는 계산에서 제외됩니다."
  (확인 절차 없음)
```
...
|---|---|---|
| **DB** | **없음** — 스키마 변경 불필요 | 0 |
| **Service** | **없음** — `BatchImportService` 재사용 | 0 |
| **API (조회)** | 신규 `GET /uploads/purchases/existing?year=YYYY` — 해당 기간 ACTIVE 배치 유무·건수·금액 반환 (읽기 전용) | 작음 |
| **API (저장)** | `POST /uploads/purchases` 에 확인 플래그 추가. 기존 배치가 있는데 플래그가 없으면 **409 로 거부**하고 교체 대상 정보를 돌려준다 | 작음 |
| **UI** | 저장 클릭 → 409 수신 시 팝업 → `[취소]` 는 아무것도 안 함 / `[교체]` 는 플래그를 붙여 재요청 | 작음 |
| **테스트** | 확인 없이 교체되지 않음 · 취소 시 기존 데이터 무변경 · 교체 시 이전 배치 SUPERSEDED | 중간 |

> API 를 새로 나누지 않고 **플래그 + 409** 로 처리하면, 기존 저장 엔드포인트의
> 계약이 깨지지 않으면서 "묻지 않고 교체" 를 구조적으로 막을 수 있다.

## 4.7.4 🔴 먼저 확인이 필요한 것

**"기존 데이터 전체 교체" 가 물리 삭제인가, 논리 제외인가?**

현재 구현은 **논리 제외**다. 이전 배치의 행은 DB 에 남고 `SUPERSEDED` 표시만
붙어 계산에서 빠진다. 되돌릴 수 있고 이력이 남는다는 장점이 있다.

| 방식 | 현재 | 특징 |
|---|---|---|
| 논리 제외 (SUPERSEDED) | ✅ 구현됨 | 이력 보존 · 복구 가능 · DB 크기 증가 |
| 물리 삭제 | ❌ | 되돌릴 수 없음 |

⛔ **어느 쪽인지 임의로 정하지 않았다.** PM 확인 사항이다.
