...
| 항목 | 값 |
|---|---|
| 문서 종류 | 명세서 (Spec) — **구현 전 승인용** |
| 작성일 | 2026-08-09 |
| 기준 커밋 | `main = a2cd31b` |
| 대상 | 작업 A — 정책 목표율 등록 기능 |
| 구현 여부 | ❌ **구현하지 않음.** 본 문서는 설계 제안이다. |

> 본 문서의 모든 내용은 **PM 승인 전까지 구현하지 않는다.**
> 승인 후 별도 구현 PR 로 진행한다.
...
지시서 0장에 기재된 정책 코드와 `main` 의 실제 seed 코드가 다르다.

| 지시서 0장 표기 | `main` 실제 코드 (`bootstrap.MVP_POLICY_SEEDS`) | 일치 |
|---|---|---|
| `SMALL_BUSINESS` | `SMALL_BUSINESS` | ✅ |
| `WOMEN_BUSINESS` | **`WOMAN`** | ❌ |
| `STARTUP` | `STARTUP` | ✅ |
| `DISABLED_BUSINESS` | **`DISABLED`** | ❌ |
| `GREEN` | `GREEN` | ✅ |

`WOMAN` · `DISABLED` 는 **PM 이 직접 확정**하신 코드값이며(이전 지시서 "policy_code 확정"),
`bootstrap.py` 와 테스트에 그대로 반영되어 있다. 이번 지시서의 `WOMEN_BUSINESS` ·
`DISABLED_BUSINESS` 는 그 확정값과 다르다.

**임의로 변경하지 않았다.** 정책 코드는 목표율 등록 API 의 URL 경로 식별자
(`PUT /policies/{policy_code}/target-rate`)로 쓰이므로, 어느 쪽이 정본인지
**PM 확인이 필요하다** → 결정 항목 **D-15**.
...
| 요청 본문 `target_rate` | **문자열 또는 `null`** |

> ⚠️ **요청 본문에서 `target_rate` 를 문자열로 받는 이유**: JSON number 로 받으면
> Pydantic 이 `float` 를 거쳐 `Decimal` 로 변환하면서 정밀도가 손상될 수 있다.
> 응답에서 문자열로 직렬화하는 기존 규약과도 대칭이 된다.
> 단, 숫자 입력도 허용할지는 **D-16** 으로 분리한다.
...
D-11 은 **승인/반려 결정**이 필요하다. 반려 시 `null` 요청은 422 로 거부하도록 명세를 수정한다.
...
## 7.3 권장안: **A안(이번 단계) + B안(별도 Issue 로 등록)** 🟡
...
> ⚠️ A안 채택 시 알려진 한계를 명시한다: **목표율이 언제 얼마에서 얼마로 바뀌었는지
> 추적할 수 없다.** 이는 수용된 한계이지 미발견 결함이 아니다.
...
## 8.5 권장안: **현재 분모 구조를 근거로 `0 < target_rate <= 100` 을 적용하되, 정책 근거가 아니라 "구조적 제약"으로 명시한다** 🟡

명세 표기 예:

```text
검증: 0 < target_rate <= 100
근거: 정부 기준이 아니라, 현재 Calculator 의 분모(전체 구매액) 정의상
      구매비율이 100 을 넘을 수 없기 때문. 분모 구조가 다른 정책
      (Group C)을 도입할 때 이 제약을 재검토한다.
```

> **이 상한을 "정부 공식 기준"으로 표현하지 않는다.**
> PM 이 상한을 두지 않기로 결정하면 `> 0` 검증만 유지하고, 100 초과 값에 대해
> **경고(warning)를 응답에 포함**하는 대안이 있다 → 이 대안도 D-13 선택지에 포함한다.

## 8.6 D-13 선택지

| 안 | 내용 | 위험 |
|---|---|---|
| **가** (권장) | `0 < x <= 100`, 근거는 "구조적 제약"으로 명시 | Group C 도입 시 재검토 필요(이미 별도 설계 대상) |
| 나 | `> 0` 만 유지 | 오입력이 조용히 저장됨 |
| 다 | `> 0` + 100 초과 시 응답에 경고 포함, 저장은 허용 | 구현 복잡도 증가, 경고를 무시할 수 있음 |
...
PUT 요청 → FastAPI Depends(require_admin_token)
         → 헤더 X-Admin-Token 과 환경변수 값 비교
         → 불일치/미설정 시 401
GET /policies 는 읽기 전용이므로 통제 대상에서 제외(Dashboard 와 동일 수준)
```
...
| 코드·문서·GitHub 저장 | **금지** |
| 토큰 미설정 시 동작 | **쓰기 API 를 비활성(503 또는 라우트 미등록)** — "설정 안 하면 무방비"가 되지 않도록 |
| 테스트 | 테스트 전용 토큰을 fixture 에서 주입. 실제 값 하드코딩 금지 |

> 🔒 **실제 토큰 값을 코드·문서·GitHub·PR 본문에 기록하지 않는다.**

## 9.4 D-14 결정 필요 사항

- 다안 채택 여부
- 토큰 미설정 시 동작: **쓰기 비활성** / 통제 없이 허용 (전자 권장)
- 헤더명 (`X-Admin-Token` 제안)
...
| 2 | `target_rate` 키 누락 | **422** | 값 미변경과 명시적 해제를 구분(6.3) |
| 3 | `target_rate` 가 숫자 문자열이 아님 (`"abc"`, `""`) | **422** | |
| 4 | `target_rate <= 0` | **422** | **기존 `insert` 경로와 동일 규칙**(`_validate_required`) |
| 5 | `target_rate > 100` | **422** | ⚠️ **D-13 결정에 종속.** 나안 채택 시 이 규칙 제거 |
| 6 | `target_rate = null` | **200** | ⚠️ **D-11 결정에 종속.** 반려 시 422 |
| 7 | 소수 자릿수 | 소수 둘째 자리까지 허용 | 기존 `Decimal` 저장 형식과 일치 |
| 8 | 비활성 정책(`is_active = false`) 변경 | **미확정** | → **D-17** |

> 규칙 4 를 서비스가 아니라 **Repository 에서도** 적용한다. 그래야 `insert` 와
...
|---|---|
| 200 | 정상 |
| 401 | 관리자 토큰 불일치 (D-14 다안 채택 시) |
| 404 | `policy_code` 없음 |
| 422 | 검증 실패 (10장) |
| 503 | 관리자 토큰 미설정으로 쓰기 API 비활성 (D-14 결정에 종속) |

## 11.2 신규 예외

| 예외 | 매핑 |
|---|---|
| `PolicyNotFoundError` (신규) | 404 |
| `PolicyValidationError` (**기존 재사용**) | 422 |

> `PolicyValidationError` 는 이미 `policy_repository.py` 에 있다. 새로 만들지 않는다.
> 단 **기존 예외에 새 HTTP 핸들러를 붙이는 것이 기존 동작에 영향이 없는지**
> 확인이 필요하다 → 12.2.
...
| # | 항목 |
|---|---|
| 1 | `PolicyValidationError` 에 전역 예외 핸들러를 추가하면, 기존 경로에서 이 예외가 발생하던 상황의 응답이 바뀌는가? → **기존 테스트로 확인**. 바뀐다면 전역 핸들러 대신 **엔드포인트 내부에서 변환**한다 |
| 2 | `app.py` 에 라우트를 추가해도 기존 8개 `test_app.py` 테스트가 그대로 통과하는가 |
...
| A-9 | `target_rate: null` → 200 (**D-11 승인 시**) |
| A-10 | 상한 초과 → 422 (**D-13 가안 채택 시**) |
| A-11 | 토큰 없음/불일치 → 401 (**D-14 다안 채택 시**) |
| A-12 | 같은 값 2회 `PUT` → 결과 동일(멱등) |
...
| # | 항목 |
|---|---|
| 1 | 관리자 토큰은 **환경변수만** 사용. 코드·문서·GitHub·PR 본문에 기록 금지 |
| 2 | 토큰 미설정 시 쓰기 API 를 **비활성**(권장) — "설정 안 하면 무방비"를 방지 |
| 3 | 오류 응답에 토큰 값이나 환경변수명 이상의 정보를 노출하지 않는다 |
| 4 | `GET /policies` 는 목표율만 노출(개인정보·사업자번호 없음) → 통제 대상 아님 |
| 5 | 로그에 요청 본문 전체를 남기지 않는다(토큰 헤더 유출 방지) |
| 6 | 배포 형태(내부망 여부)가 문서화되어 있지 않다 → **미확정**. 9.2 가안의 전제를 검증할 수 없다 |
...
| # | 항목 | 현재 결정 |
|---|---|---|
| 1 | 변경 이력 테이블 | D-12 B안 — **별도 Issue 로 등록**(이번 미구현) |
...
| ID | 결정 사항 | 권장안 | 확신도 |
|---|---|---|---|
| **D-11** | `NULL` 로 되돌리기 허용 | **허용** | 높음 — 시스템이 이미 `NULL` 을 정상 상태로 정의 |
| **D-12** | 변경 이력 | **A안(`updated_at`) + B안은 별도 Issue** | 중간 — D-14 미정이면 B안의 가치가 반감 |
| **D-13** | 목표율 상한 | **`<= 100`, 단 "구조적 제약"으로 명시** | 중간 — 정부 근거 아님. 숫자의 출처를 반드시 표기 |
| **D-14** | 쓰기 API 접근 통제 | **다안(환경변수 토큰)** | 높음 — 가안은 배포 형태 미확인, 나안은 지시서가 금지 |

## 16.2 이번 분석에서 새로 발견되어 추가된 결정

| ID | 결정 사항 | 비고 |
|---|---|---|
| **D-15** | **정책 코드 정본** — `WOMAN`/`DISABLED`(현 코드) vs `WOMEN_BUSINESS`/`DISABLED_BUSINESS`(이번 지시서) | 🔴 **API URL 식별자이므로 구현 전 확정 필요.** 변경 시 seed·테스트·문서 전반 영향 |
| **D-16** | 요청 본문의 `target_rate` 를 문자열만 받을지, 숫자도 허용할지 | 정밀도 보존과 사용 편의의 절충 |
| **D-17** | 비활성 정책(`is_active = false`)의 목표율 변경 허용 여부 | 현재 비활성 정책은 없으나 규칙은 필요 |
...
|---|---|
| 정책 코드 정본(D-15) | PM 확인 |
| 목표율 상한의 **정책적** 근거 | 정부 자료 (현재는 구조적 근거만 있음) |
| 목표율이 연도별로 바뀌는지 | 정부 자료 |
| 배포 형태(내부망 여부) | 운영 환경 정보 |
