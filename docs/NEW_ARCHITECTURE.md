...
| 순위 | 단계 | 선행 조건 | 근거 |
|---|---|---|---|
| **1** | DB-2 스키마 확정 + Repository | PM 승인 | 분석·검토 없이는 다음이 없다. 기존 코드에 영향 0 |
| **2** | 분석 인터페이스(추상) + 규칙 없는 기본 구현 | 1 | 인터페이스를 먼저 고정하면 방법 선택이 늦어도 된다 |
| **3** | 검토 API + 화면 | 1·2 | 담당자가 실제로 쓸 수 있어야 데이터가 쌓인다 |
