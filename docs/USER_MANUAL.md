...
> **[화면 캡처 위치] 설치 옵션 선택 화면**
> — 「이 컴퓨터를 사용하는 모든 사람」과 「전용」 중 고르는 화면입니다.
...
| 상황 | 해결 |
|---|---|
| 설치파일이 열리지 않음 | 파일이 온전히 전달됐는지 확인해 주십시오. 전달자에게 SHA-256 값을 받아 대조할 수 있습니다 |
...
> **화면 위쪽의 `DEMO / SAMPLE DATA` 표시**
> 「지금 보이는 숫자는 실제 업무 자료가 아닐 수 있다」는 뜻입니다.
> 처음에는 일부러 이렇게 켜 둡니다 — 연습용 자료를 실제로 잘못 아는 것이
> 더 위험하기 때문입니다.
> 실제 자료를 모두 올리신 뒤 바꾸는 방법은 15장에 있습니다.
...
![자료 관리 탭](images/manual-02-자료관리.png)

*자료 관리 탭 — 목표비율 관리 · 구매실적 업로드 · 기업정보 등록이
위에서 아래로 놓여 있습니다.*
...
위 **자료 관리** 탭 화면(7장)의 맨 아래쪽이 기업정보 등록 부분입니다.

> **[화면 캡처 위치] 업로드 결과 화면**
> — 실제로 파일을 올린 뒤의 결과 화면입니다. 운영 자료를 처음
>   올리실 때 캡처해 두시면 다음 담당자에게 도움이 됩니다.
...
정책마다 「기관 전체 구매액 중 몇 % 를 이 정책 기업에 써야 하는가」를
정합니다.
...
> **비워 두면 「미설정」입니다 — 0% 가 아닙니다.**
> 미설정인 정책은 달성률을 계산하지 않습니다.

> **연도마다 따로 관리됩니다.**
> 2026년 값을 바꿔도 2025년 값은 그대로입니다.

> **여성기업은 구매유형마다 목표가 다릅니다.**
> 공사 3% · 용역 5% · 물품 5% 입니다.
...
![점검 탭](images/manual-03-점검.png)

*점검 탭 — 데이터 요약과 적재 현황. 구매유형 검토 표는 이 아래에
이어집니다.*
...
**Q. 화면에 계속 `DEMO / SAMPLE DATA` 가 떠 있습니다.**

실제 자료를 다 올리신 뒤에 바꾸실 수 있습니다.
표시만 바뀌고 계산에는 아무 영향이 없습니다.

바꾸는 방법은 이 문서 맨 뒤 **[관리자 참고]** 에 있습니다.
어려우시면 담당자에게 요청해 주십시오.
...
점검 탭에서 남은 건수를 확인하고 10장대로 확정해 주십시오.
...
### DEMO 표시를 OPERATIONAL 로 바꾸기

실제 자료를 모두 올린 뒤, PowerShell 에서 아래 한 줄을 실행하고
프로그램을 다시 켜면 됩니다.
