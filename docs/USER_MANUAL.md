...
```
 ...
 ② 기업정보 등록        자료 관리 탭   ← 정책마다 한 번씩
        ↓
 ③ 구매실적 올리기      자료 관리 탭   ← 매월 (그 해 누적본을 연 단위로)
        ↓
 ④ 점검                점검 탭        ← 미적재·미매칭 확인
        ↓
...
기관의 지출 내역을 시스템에 넣습니다. 그 해 누적 원본을 연 단위로
올립니다.
...
**⭐ 받으신 원본을 그대로 올려도 되는 경우**

회계 시스템에서 내려받은 원본을 **고치지 않고** 올릴 수 있는 경우가
있습니다. 아래 두 가지는 이름이 달라도 시스템이 알아봅니다.
...
그리고 **비어 있어도 되는 네 칸(계약일자·지급일·적요·예산과목)은 칸
자체가 없어도** 됩니다.

> ⭐ 그래서 「결의일자 · 거래처명 · 사업자번호 · 계 · 신고기준일」만
> 들어 있으면 원본을 그대로 올리실 수 있습니다. 매달 손으로 옮겨
> 적지 않으셔도 됩니다.

> ⛔ 그 밖의 이름(업체명 · 상호 · 거래처 등)은 알아보지 못합니다.
> 실제로 확인한 이름만 넣어 두었습니다. 쓰시는 원본의 칸 이름이
> 다르면 알려 주십시오 — 짐작해서 넣으면 엉뚱한 칸을 읽게 됩니다.

> ⛔ 원본 파일은 **바뀌지 않습니다.** 읽을 때만 이름을 옮겨 보는
> 것이며, 시스템이 원본을 고쳐 저장하는 일은 없습니다.
...
③ **대상 연도** 를 고르고, **대상 월** 은 **「연 전체」** 로 둡니다.

> ⭐ **구매실적은 연 단위로 올립니다.**
> 회계 시스템에서 받는 파일이 보통 **그 해 처음부터 지금까지 쌓인
> 누적 자료**이기 때문입니다. 1~6월 누적본을 올리셨다면, 다음에는
> 1~7월 누적본을 같은 방법으로 올리시면 됩니다.
>
> 달을 골라 그 달만 올리는 방법도 있습니다. 다만 **두 방법을 섞으면**
> 기간이 겹쳐 등록되지 않습니다. 기관에서 한 가지로 정해 쓰십시오.
...
**⭐ 자동으로 빠지는 두 가지 — 오류가 아닙니다**

원본에는 거래 한 건이 아닌 줄이 섞여 있기도 합니다. 그런 줄 때문에
파일 전체가 막히지 않도록, 아래 두 가지는 **자동으로 빼고** 몇 건을
뺐는지 결과에 적어 드립니다.

| 안내 문구 | 무슨 뜻인가 | 왜 빼나 |
|---|---|---|
| 「집계 행 N건을 거래자료에서 제외했습니다」 | 소계·합계 줄입니다. 거래처·사업자번호·결의일자·신고기준일이 **네 칸 모두** 비어 있고 금액만 있는 줄 | 넣으면 그만큼 실적이 부풀려집니다 |
| 「결의일자가 앞선 해인 N건은 그 해 실적으로 귀속되어 이번 등록에서 제외했습니다」 | 12월 말에 결의하고 이듬해 1월에 신고한 건입니다 | 실적 연도는 **결의일자**로 정하므로 그 해 실적입니다 |

> ⭐ **원본을 손보지 않으셔도 됩니다.** 위 줄들을 매달 지우고 올리실
> 필요가 없습니다.

> ⭐ 앞선 해로 빠진 건은 **없어지지 않습니다.** 그 해를 **대상 연도**로
> 골라 올리시면 그때 들어갑니다.

> ⚠️ 네 칸 중 **하나라도 값이 있으면** 집계 행으로 보지 않고 그대로
> 검사합니다. 금액이 크다고 해서, 적요가 「합계」라고 해서 빼지
> 않습니다.

> ⚠️ **다른 달·다른 해 파일을 잘못 고르신 경우**는 예전과 같이 파일
> 전체를 등록하지 않고 알려 드립니다. 조용히 빼면 그 달 실적이 들어간
> 줄 아시게 되기 때문입니다.
...
> **같은 해를 다시 올리면 그 해 자료가 교체됩니다.**
> 더해지지 않습니다. 14장을 먼저 읽어 주십시오.
...
> 키가 없으면 조회할 수 없다는 안내가 나옵니다. 발급받는 방법은
> 바로 아래 **[9] 조회용 인증키 발급받기** 에 적어 두었습니다.
...
「조회」 방식은 **국가가 운영하는 공공데이터포털** 의 자료를 그때그때
불러오는 기능입니다. 포털은 **누가 부르는지** 를 확인하기 위해
기관마다 다른 **인증키** 를 내줍니다. 그래서 프로그램을 설치했다고
바로 조회가 되지는 않고, **기관이 직접 한 번 발급**받아야 합니다.

> ⛔ 인증키는 설치파일 안에 들어 있지 않습니다. 넣어 두고 배포하면
> 여러 기관이 한 키를 나눠 쓰게 되어, 포털의 호출 한도가 남의
> 사용량으로 차 버리고 이용 책임도 가려지지 않습니다.

**[1] 무엇을 신청하나요**

쓰시려는 조회 출처에 따라 신청할 자료가 다릅니다.

| 쓰려는 조회 출처 | 신청할 자료 | 신청 주소 |
|---|---|---|
| 여성기업 · 장애인기업 · 창업기업(중소기업 확인) | 공공구매종합정보망 인증서 정보 제공 서비스 | https://www.data.go.kr/data/15062581/openapi.do |
| 창업기업(창업진흥원) | 창업기업확인서 발급기업정보 조회서비스 | https://www.data.go.kr/data/15125362/openapi.do |

> ⭐ 위쪽 하나만 신청해도 **여성 · 장애인 · 창업(중소기업 확인)**
> 세 가지 조회를 모두 쓰실 수 있습니다. 창업기업을 창업진흥원
> 자료로도 확인하고 싶을 때만 아래쪽을 추가로 신청하십시오.

**[2] 발급 순서**

```
① 공공데이터포털(data.go.kr) 회원가입 · 로그인
        ↓
② 위 표의 신청 주소로 들어갑니다
        ↓
③ [활용신청] 을 누릅니다
...
④ 활용 목적과 담당자 정보를 적습니다
        ↓
⑤ 승인 (자동 승인되거나, 담당자 검토를 거칩니다)
        ↓
⑥ 포털 [마이페이지] → [데이터활용] → [개발계정] 에서
   인증키를 확인합니다
```

④ 에 적으실 내용은 아래를 참고하십시오.

| 칸 | 적으실 내용 |
|---|---|
| 활용 목적 | 공공기관 우선구매 정책 달성률 산정 |
| 활용 형태 | 기관 내부 시스템에서 조회 (재배포 없음) |
| 담당자 | 기관 담당자 이름 · 연락처 · 이메일 |

**[3] ⚠️ 어느 키를 복사하나요 — 여기서 가장 많이 틀립니다**

마이페이지에는 인증키가 **두 줄** 나옵니다.

| 이름 | 생김새 | 쓰나요 |
|---|---|---|
| **일반 인증키 (Decoding)** | `abcd...==` 처럼 `+` `/` `=` 가 섞여 있음 | ✅ **이것을 씁니다** |
| 일반 인증키 (Encoding) | `abcd...%3D%3D` 처럼 `%` 가 섞여 있음 | ❌ 쓰지 않습니다 |

> **Decoding** 쪽을 복사하십시오. Encoding 키를 넣으면 조회가
> 「인증키가 올바르지 않습니다」로 실패합니다. 두 키는 **같은 값을
> 다르게 적은 것** 이라 눈으로는 비슷해 보입니다.

**[4] 받은 키를 넣기**

받으신 키는 **설치된 컴퓨터에 한 번만** 넣어 두면 됩니다.
넣는 위치와 방법은 맨 뒤 **[관리자 참고] → 조회 방식의 인증키 설정**
에 적어 두었습니다. 전산 담당자에게 그 쪽을 보여 주시면 됩니다.

**[5] 승인까지 걸리는 시간**

포털과 제공기관 사정에 따라 다릅니다. **신청 즉시 되는 경우도 있고,
며칠 걸리는 경우도 있습니다.** 승인 상태는 포털 마이페이지에서
확인하실 수 있습니다.

> ⭐ 승인을 기다리는 동안에도 **파일 업로드** 방식은 그대로 쓰실 수
> 있습니다. 평소 업무에 필요한 것은 파일 업로드 쪽입니다.
...
## 구매실적 — 같은 해를 다시 올리는 경우

### 어떻게 되나요?

**그 해의 자료가 통째로 교체됩니다.**

① 같은 연도로 다시 올리면 시스템이 먼저 묻습니다.

② 승인하시면 **이전 자료 대신 새 자료를 씁니다.**

③ 다른 연도는 그대로 둡니다.

> ⭐ 매달 이 방법으로 올리시면 됩니다. 회계 시스템에서 받는 파일이
> 누적본이므로, 새 누적본이 이전 누적본을 대신하는 것입니다.
>
> (달을 골라 올리는 방식으로 운영하신다면 「그 해」를 「그 달」로
> 바꿔 읽으시면 됩니다. 그 달만 교체되고 다른 달은 그대로입니다.)

### 예전에 월별로 올리신 자료가 있다면

**그 해 전체를 올리면, 그 해 안에 있던 예전 등록도 함께 교체됩니다.**

예를 들어 3월치를 따로 올려 두신 상태에서 1~6월 누적본을 「연 전체」로
올리시면, 3월 등록도 함께 교체됩니다. 누적본 안에 3월 거래가 이미
들어 있기 때문입니다.

시스템이 **먼저 묻습니다.** 무엇이 함께 바뀌는지 알려 드리고, 승인하셔야
바뀝니다.

> ⭐ 예전 등록은 **지워지지 않습니다.** **업로드 이력** 에 「대체됨」으로
> 남습니다.

> ⚠️ **반대는 되지 않습니다.** 그 해 전체를 올려 두신 상태에서 한 달치만
> 올리시면 등록되지 않고 안내가 나옵니다. 그렇게 하면 나머지 달이 통째로
> 빠지기 때문입니다. 그 해 전체를 다시 올려 주십시오.

> ⚠️ **일부만 겹치는 경우도 등록되지 않습니다.** 예를 들어 1~6월 등록이
> 있는데 3~7월짜리 파일을 올리는 경우입니다. 어느 쪽으로 처리해도 거래가
> 빠지거나 두 번 세어지므로, 기간을 맞춰 다시 올려 주십시오.

### ⚠️ 주의

> **더해지는 것이 아니라 바뀝니다.**
> 빠진 건만 추가로 올리시면 안 됩니다. **누적 원본 전체**를 올려
> 주십시오.
...
- [ ] 그 해 자료가 최신 누적본으로 올라가 있는가 (업로드 이력)
- [ ] 미적재 행이 남아 있지 않은가
- [ ] 구매유형 확정이 끝났는가
- [ ] 「계산 보류」·「조회불가」 정책이 있는가
- [ ] 있다면 **0% 로 적지 않았는가**
...
기업정보 「조회」 방식(8장)을 쓰시려면 발급받은 인증키를 사용자 데이터
폴더의 `.env` 에 넣어야 합니다. 발급받는 절차는 **8장 [9] 조회용
인증키 발급받기** 에 있습니다.

| 넣을 이름 | 어느 조회에 쓰이나 |
|---|---|
| `SMPP_API_KEY` | 여성기업 · 장애인기업 · 창업기업(중소기업 확인) |
| `STARTUP_API_KEY` | 창업기업(창업진흥원) |

쓰지 않는 조회의 키는 넣지 않아도 됩니다.
...
> ⚠️ 포털 마이페이지의 **일반 인증키(Decoding)** 를 붙여 넣으십시오.
> `%` 가 섞인 Encoding 키를 넣으면 조회가 실패합니다.
