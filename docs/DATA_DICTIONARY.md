...
## 2.6 ⚠️ 명명 규칙 확인 요청 (PM 결정 필요)

PM 지시서의 예시와 **현재 구현된 물리명이 일부 다르다.** 아래 두 건은 확인이 필요하다.

| 논리명 | PM 예시 | **현재 구현** | 비고 |
|---|---|---|---|
| 사업자등록번호 | `business_registration_number` | **`business_no`** | 4개 테이블·Repository·테스트에 이미 적용 |
| 구매금액 | `purchase_amount` | **`amount`** (Purchase 테이블) | `purchase_amount`는 **API 응답의 "정책별 구매액"** 으로 이미 사용 중 |
| 지급일 | `payment_date` | `payment_date` | ✅ 일치 |

### 권고

- **현재 물리명(`business_no`, `amount`) 유지를 권장**한다.
  - 변경 시 모델·Repository·계산기·API·테스트 전반을 수정해야 하며, 기능 이득은 없다.
  - 특히 `purchase_amount`는 이미 **다른 의미(정책별 구매액)** 로 API에 노출되어 있어,
    Purchase 테이블 컬럼명으로 재사용하면 **의미 충돌**이 발생한다.
- 다만 PM 판단으로 명명 통일이 필요하다면, **Sprint B(구축) 착수 시 일괄 변경**하는 것이
  안전하다. Sprint A 종료 전에 결정해 주시기 바란다.
