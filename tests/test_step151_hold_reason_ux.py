...
해야 할 일은 서로 다르다.
    ...
    구매유형 확인 중   확인을 끝내면 계산된다        → 구매유형 검토
...
그 화면에 가도 **할 일이 없었다.** 실제로 해야 할 일은 구매유형 검토에서
...
class TestTheNextStepGoesWhereTheWorkIs:
    ...
    def test_purchase_type_goes_to_the_review_card(self) -> None:
        ...
        # ② 요구사항 변경(STEP 156.6) — 화면 이름을 「구매유형 검토」로
        #    통일했다. 예전에는 단추만 「구매실적 검토」라 그 이름의 화면을
        #    찾을 수 없었다.
        assert 'go: "구매유형 검토"' in review
